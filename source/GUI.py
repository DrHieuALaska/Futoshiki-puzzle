...
class FutoshikiApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # Speed selector
        self.speed_menu = tk.Menubutton(hbtn, textvariable=self.speed_var, bg=BTN_BG, fg=BTN_FG,
                                        activebackground=BTN_ACT, activeforeground=BTN_FG,
                                        font=FONT_UI, relief="flat", bd=0, padx=14, pady=7, cursor="hand2")
        ...
        self.replay_btn = self._btn(hbtn, "▶ Replay", self._replay_history)
        self.replay_btn.pack(side="left", padx=4, pady=10)
        self.replay_btn.config(state="disabled")
        
        self.stop_replay_btn = self._btn(hbtn, "⏹ Stop Replay", self._stop_replay)
        # Initially hidden
        
        self.resume_replay_btn = self._btn(hbtn, "⏸ Resume", self._resume_replay)
        # Initially hidden
    ...
    def _build_stats_panel(self, parent):
        ...
        # History section
        tk.Frame(inner, bg=BORDER, height=1).pack(fill="x", pady=(12, 12))
        tk.Label(inner, text="HISTORY", font=("Segoe UI", 8, "bold"),
                 bg=STAT_BG, fg=ACCENT, anchor="w").pack(fill="x", pady=(0, 12))

        self.history_text = scrolledtext.ScrolledText(inner, height=10, width=20, bg=STAT_BG, fg=STAT_VALUE, font=("Courier New", 8))
    ...
    def _reset(self):
        ...
        # Stop/pause replay if running or paused
        self.replay_running = False
        self.replay_paused = False
        
        # Restore puzzle grid to initial state (only given values)
        if hasattr(self, 'initial_grid'):
            self.puzzle.grid = [row[:] for row in self.initial_grid]
        
        # Reset grid display to initial state
        for i in range(self.puzzle.N):
            for j in range(self.puzzle.N):
                # Only show given values, clear all solved values
        ...
        # Clear all history and replay memory
        self.history = []
        self.replay_index = 0
        self.replay_btn.config(state="disabled")
        
        # Hide all replay-related buttons and restore replay button
        self.stop_replay_btn.pack_forget()
        self.resume_replay_btn.pack_forget()
        self.replay_btn.pack(side="left", padx=4, pady=10)
        
        # Clear history text display
        if self.history_text:
            self.history_text.config(state="normal")
            self.history_text.delete(1.0, tk.END)
            self.history_text.insert(tk.END, "No history yet")
        ...
        # Reset all statistics display
        self._reset_stats()
        self.stat_vars["size_disp"].set(f"{self.puzzle.N}×{self.puzzle.N}")
        self.stat_vars["status_disp"].set("Ready")
        
        # Update status and time display
        self._set_status("Board reset to initial state", "info")
        self.time_var.set("")
    ...
    def _replay_history(self):
        ...
        # Hide Replay button and show Stop Replay button
        self.replay_btn.pack_forget()
        self.stop_replay_btn.pack(side="left", padx=4, pady=10)
        
        self._next_replay_step()

    def _next_replay_step(self):
        # Check if replay was stopped
        if not self.replay_running:
            return
        
        if self.replay_index >= len(self.history):
            self._set_status("Replay finished", "ok")
            self.replay_running = False
            # Show Replay button and hide Stop Replay button
            self.stop_replay_btn.pack_forget()
            self.replay_btn.pack(side="left", padx=4, pady=10)
            return
        
        action, r, c, val = self.history[self.replay_index]
        if action == 'assign':
            self.puzzle.grid[r][c] = val
            self.cell_vars[r][c].set(str(val))
            if self.given_mask[r][c]:
                self.cell_wdgs[r][c].config(bg=GIVEN_BG, fg=GIVEN_FG)
            else:
                self.cell_wdgs[r][c].config(bg=CELL_BG, fg=CELL_FG)
        elif action == 'clear':
            ...
            if self.given_mask[r][c]:
                ...
                self.cell_wdgs[r][c].config(bg=CELL_BG, fg=CELL_FG)
        self.replay_index += 1
        speed_factor = float(self.speed_var.get().replace('x', ''))
        delay = int(200 / speed_factor)  # Base 200ms
        self.after(delay, self._next_replay_step)
    ...
    def _stop_replay(self):
        ...
        self._set_status("Replay paused", "warn")
        # Hide Stop Replay button and show Resume button
        self.stop_replay_btn.pack_forget()
        self.resume_replay_btn.pack(side="left", padx=4, pady=10)
    ...
    def _resume_replay(self):
        ...
        self.replay_running = True
        self.replay_paused = False
        self._set_status("Replay resumed", "info")
        # Hide Resume button and show Stop Replay button
        self.resume_replay_btn.pack_forget()
        self.stop_replay_btn.pack(side="left", padx=4, pady=10)
    ...
    def _solve(self):
        ...
        # Ensure puzzle is in clean state (only with given values)
        if hasattr(self, 'initial_grid'):
            self.puzzle.grid = [row[:] for row in self.initial_grid]
    ...
    def _show_result(self, solution, elapsed, algo, stats, history=None):
        ...
        # Save history and enable replay
