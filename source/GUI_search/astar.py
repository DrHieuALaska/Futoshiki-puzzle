...
def solve_astar(puzzle, kb, heuristic='ac3'):
    ...
    if not valid:
        ...
        tracemalloc.stop()
        return None, _make_stats(0, 0, start_time, peak), history

    # Already solved after initial propagation?
    if(is_finished):
        solution, history = _extract_solution(puzzle, domains_0, history)
        _, peak  = tracemalloc.get_traced_memory()
        tracemalloc.stop()
        return solution, _make_stats(1, 0, start_time, peak), history
    ...
    while open_list:
        f, g, _, curr_puz, curr_facts, curr_dom, curr_history = heapq.heappop(open_list)
        ...
        if all(len(d) == 1 for d in curr_dom.values()):
            solution, history = _extract_solution(curr_puz, curr_dom, curr_history)
            _, peak  = tracemalloc.get_traced_memory()
            tracemalloc.stop()
            return solution, _make_stats(expansions, generated, start_time, peak), history
        ...
        for val in sorted(cell_dom):   # sorted for determinism
            ...
            if is_finished:
                ...
                _, peak  = tracemalloc.get_traced_memory()
                tracemalloc.stop()
                return solution, _make_stats(expansions + 1, generated + 1, start_time, peak), history
            ...
            heapq.heappush(open_list, (new_g + new_h, new_g, tie, succ_puz, succ_facts, succ_dom, succ_history))
            tie       += 1
            generated += 1
    ...
    return None, _make_stats(expansions, generated, start_time, peak), history


def _extract_solution(base_puzzle, domains, history):
    ...
    for (i, j), vals in domains.items():
        ...
        history.append(('assign', i, j, vals))
